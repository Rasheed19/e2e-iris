...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def get_prepared_data():
        if input.prediction_type() == "Single-sample prediction":
            return prepare_single_prediction_data(
        ...
        return prepare_batch_prediction_data(uploaded_data=parsed_file())
    ...
    def prediction_table():
        ...
        if isinstance(prepared_data, pd.DataFrame):
            ...
            @render.download(
                filename=lambda: f"iris-prediction-{date.today().isoformat()}.csv",
                media_type="text/csv",
            )
